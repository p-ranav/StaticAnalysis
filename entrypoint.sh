...
if [ -z "$INPUT_EXCLUDE_DIR" ]; then
    eval cppcheck --suppressions-list=suppressions.txt --language=c++ "$INPUT_CPPCHECK_ARGS" --output-file=cppcheck.txt .
    run-clang-tidy-12 -- >(tee "clang_tidy.txt")
else
    eval cppcheck --suppressions-list=suppressions.txt --language=c++ "$INPUT_CPPCHECK_ARGS" --output-file=cppcheck.txt  -i"$GITHUB_WORKSPACE/$INPUT_EXCLUDE_DIR" .
